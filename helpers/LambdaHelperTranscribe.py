...
class Lambda_Helper_transcribe: 
    ...
    def __init__(self):
        ...
        self.role_arn = f"arn:aws:iam::{account_id}:role/AdministratorE2"
    ...
    def deploy_function(self, code_file_names, function_name="", handler="lambda_function.lambda_handler"):
        ...
        zip_file_path = handler +'lambda_function.zip'

        with zipfile.ZipFile(zip_file_path, 'w') as zipf:
            for code_file_name in code_file_names:
                # This maintains the subdirectory structure within the ZIP file
                zipf.write(code_file_name, arcname=os.path.join(os.path.dirname(code_file_name), os.path.basename(code_file_name)))
